...
**NEVER** use `uv run --with pytest pytest -c "..."` or inline expressions.

For running pytest (always use script-style, not inline):

```bash
uv run --with pytest --with hypothesis pytest path/to/tests/ -v
```
For running a single test file:

```bash
uv run --with pytest --with hypothesis pytest callbot/module/tests/test_foo.py -v
```
